    ...
    methods: {
        ...
        async searchDataAsync() {
            ...
            try {
                ...
                this.isLoaded = false;
                const response = await axios.get("/api/common/post/search?search=" + this.searchString + "&page=1");
                this.postList = response.data;
    ...
    params: {
      id: "-1",
        ...
        async getDataChildAsync(myId:string)
        {
            debugger;
            this.isLoaded = false
            var responce = await axios.get('/api/common/post/user/' + myId + '?page=1')
...
<template>
    ...
    <ul class="grid grid-cols-3 md:grid-cols-3 gap-5">
        <template v-for="item in postList">
            <PostViewComponent :categoryId=item.categoryId :userId=item.userId :title=item.title :price=item.price
